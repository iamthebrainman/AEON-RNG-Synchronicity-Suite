...
The heart of the AEON suite is the **Random Number Visualizer** (`RandomNumberVisualizer.py`), a sophisticated dashboard for real-time entropy analysis.

### Features:
- **Triple-View Visualization**:
...
- **Intelligent Anomaly Engine**:
  ...
  - **Pattern Repetition**: Tracks hex signatures to identify repeating "motifs" in the data stream.
...
### Features:
- **Adaptive Oracle**: An experimental predictor that utilizes feedback loops to learn mappings between RNG states and targets.
...
```
- **Start/Stop**: Control the real-time entropy stream.
- **🔮 Calibration**: Open the multi-dimensional search engine to find high-resonance channels.
- **Note-Taking**: Add timestamped markers to your session logs for easier correlation with mental focus or external events.
